...
rm -rf BloodyMary.xcodeproj
# run xcodegen to generate new file
xcodegen
# move config files to right place
cp Configurations/IDETemplateMacros.plist BloodyMary.xcodeproj/xcshareddata/IDETemplateMacros.plist

# if argument "open" passed, open the project
if [[ $1 == "open" ]]; then
    open BloodyMary.xcodeproj
fi
